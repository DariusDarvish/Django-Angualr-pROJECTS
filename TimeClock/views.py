...
def register(request):
    ...
    if request.method == 'POST':
        ...
        #how tto claim vlaues
        res = request.POST.get('Admin')
        res1= request.POST.get('Contractor')

        if res:
            #get the value of class equals radio button
        ...
        if res1:
            ...
            # if else statment
            u = User.objects.get(username=username)
            permission = Permission.objects.get(codename='Contractor')
            print('user')
            u.user_permissions.add(permission)
            print('user created')
            return redirect('/')
